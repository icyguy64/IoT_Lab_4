...
certificate_formatter = "device_{}.certificate.pem.crt"
...
class MQTTClient:
    def __init__(self, device_id, cert, key):
        ...
        self.client.configureEndpoint("a1e2dljenok9dk-ats.iot.ap-northeast-1.amazonaws.com", 8883)
...
counter = 1
while True:
    #print(data[0]["vehicle_CO2"][1])
    print("send now?")
    #x = input()
    if 1:#x == "s":
        for i,c in enumerate(clients):
        	c.publish(json.dumps(
        	{"message":"{}".format(data[i]["vehicle_CO2"][counter])}
        	),
        	)
